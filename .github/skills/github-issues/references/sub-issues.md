...
```graphql
mutation {
  createIssue(input: { repositoryId: "REPO_NODE_ID", title: "Implement login validation", parentIssueId: "PARENT_NODE_ID" }) {
    issue {
      id
      number
    }
...
```graphql
mutation {
  reprioritizeSubIssue(input: { issueId: "PARENT_NODE_ID", subIssueId: "CHILD_NODE_ID", afterId: "OTHER_CHILD_NODE_ID" }) {
    issue {
      id
    }
  }
