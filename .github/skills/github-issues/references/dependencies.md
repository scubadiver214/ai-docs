...
```graphql
mutation {
  addBlockedBy(input: { issueId: "BLOCKED_ISSUE_NODE_ID", blockingIssueId: "BLOCKING_ISSUE_NODE_ID" }) {
    blockingIssue {
      number
      title
    }
...
```graphql
mutation {
  removeBlockedBy(input: { issueId: "BLOCKED_ISSUE_NODE_ID", blockingIssueId: "BLOCKING_ISSUE_NODE_ID" }) {
    blockingIssue {
      number
      title
    }
