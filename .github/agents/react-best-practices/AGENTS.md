...
Use a module-level registry to share global event listeners across component instances.
...
function useKeyboardShortcut(key: string, callback: () => void) {
  useEffect(() => {
    ...
    if (!globalListener) {
      ...
      window.addEventListener('keydown', handler);
      globalListener = () => window.removeEventListener('keydown', handler);
    }
    ...
    return () => {
      ...
      if (keyCallbacks.size === 0 && globalListener) {
        globalListener();
        globalListener = null;
      }
    };
...
TanStack Query enables request deduplication, caching, background refetching, and revalidation across component instances. All HTTP requests MUST use the native `fetch` API through the API service layer.
...
**Correct: multiple instances share one request via TanStack Query**

```tsx
// src/api/users/api.ts — Pure fetch function
...
// src/api/users/hooks/useFetchUsers.ts — TanStack Query wrapper
...
**Use query key factories for structured invalidation:**

```typescript
// src/api/users/query-keys.ts
...
// Targeted invalidation — only list queries, not detail queries
queryClient.invalidateQueries({ queryKey: userKeys.lists() });
```

**Use `useSuspenseQuery` with `<Suspense>` for streaming performance:**

Instead of handling `isLoading` manually in every component, use `useSuspenseQuery` and let `<Suspense>` boundaries handle loading states. This enables streaming and eliminates loading state branching in components.

```tsx
// Incorrect: manual loading check, blocks streaming
function UserList() {
...
// Correct: Suspense handles loading, component always has data
...
// Parent wraps with Suspense — enables streaming
<Suspense fallback={<Skeleton />}>
  <UserList />
</Suspense>;
```
