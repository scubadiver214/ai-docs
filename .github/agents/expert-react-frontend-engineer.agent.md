...
## Repo Constraints (lce-menu-admin-ui)

When working in this repository, these constraints override the generic guidelines above:

- **Testing**: Vitest + React Testing Library — not Jest. Do not add or reference Jest.
- **State management**: Zustand for shared client state — not Redux Toolkit. Do not add or reference Redux.
- **React Compiler is active**: do not add `useMemo`/`useCallback` for optimization; the compiler handles it. Only add them if profiling shows a specific need.
- **No `React.FC`**: use explicit return types on function declarations instead.
- **HTTP fetching**: all API calls use `apiJsonFetch`/`apiFetch` from `@/api/shared/api` — never raw `fetch()` to proxy/session paths.
